...
MakePendulumPlant(const PendulumParameters& params,
  ...
  auto plant = std::make_unique<multibody::MultibodyPlant<double>>(params.dt());
  ...
  multibody::UnitInertia<double> G_Bo =
      multibody::UnitInertia<double>::PointMass(p_BoBcm_B);
  multibody::SpatialInertia<double> M_Bo(params.m(), p_BoBcm_B, G_Bo);

  // Add a rigid body to model the point mass.
  const multibody::RigidBody<double>& point_mass =
  ...
  const multibody::RevoluteJoint<double>& pin = plant->AddJoint<multibody::RevoluteJoint>(
